...
for(let i = 0; i < artists.length; i++){
  if(artists[i].name ){
   artistLookUp.push(artists[i]);
  }
}
console.log(artistLookUp[0].name)

let artistBioLookUp = []
for(let i = 0; i < artists.length; i++){
  if(artists[i].bio ){
   artistBioLookUp.push(artists[i]);
  }
}
console.log(artistBioLookUp[2].bio)
...
bio:'Mediocre artist, tried very hard but was ultimately defeated by her own over thinking. Survivrd by her two very artistic daughters'};
